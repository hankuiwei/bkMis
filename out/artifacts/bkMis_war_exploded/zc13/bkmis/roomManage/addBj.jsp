...
<html>
	<head>
		...
		document.onkeydown = function(evt){
		  ...
		  function selectbelong(type){
		       ...
		       $.post("<%=path%>/room.do",{method:"getMeterByBuildIdAndName",type:type,buildId:buildId},function(data){
		                var str1 = data.split(";");
		  ...
		  //在页面一打开时就开始加载,用于控制所属总表是否显示(现在没有在使用)
		  function getBelong(){
		      var buildId = document.getElementById("buildId").value;
		      //到后台根据楼栋id查询表具信息，如果有就显示
		      $.post("<%=path%>/room.do",{method:"getMeterByBuildId",buildId:buildId},function(data){
		          if(data == 'true'){
	...
	<body>
		<form method="post" name="addMeterForm" action="<%=path%>/room.do?method=addMeter">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td align="center">
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td height="10" colspan="9"><input type="hidden" id="buildId" name="buildId" value="${buildId }"></td>
										</tr>
										<tr>
