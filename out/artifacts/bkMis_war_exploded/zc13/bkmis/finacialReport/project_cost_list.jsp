...
<head>
	...
	<script type="text/javascript">
		...
		function add(){
			var url = "<%=path %>/financialReport.do?method=addOrEditProjectPage";
		...
		function edit(){
			...
			var url = "<%=path %>/financialReport.do?method=addOrEditProjectPage&projectCosts.id="+id;
			var options = "dialogWidth:800px;dialogHeight:600px;status:no;scroll:no;";
			var win = window.showModalDialog(url, this.window, options);
			if(win=="flag"){
				query();
		...
		function query(){
			document.form1.action="<%=path %>/financialReport.do?method=getProjectCostList";
		...
		function del(){
			...
			if(confirm("确定要删除这"+k+"条记录？")){
				document.form1.action = "<%=path %>/financialReport.do?method=delProjectById&ids="+str;
				document.form1.submit();
			}
		}
	</script>
    		...
    		<td>
    			<table width="100%" border="0" cellspacing="0" cellpadding="0">
      				<tr>
						...
						<td width="165" nowrap="nowrap" class="form_line">项目费用列表</td>
								...
								<tr height = "95%">
					        		...
					        		<div id = "div1" class = "RptDiv"  >
							   			<table border="0" cellpadding="0" cellspacing="0" class = "RptTable">
											...
											<c:choose>
											<c:when test="${empty projectCostList}">
											<tr align="center">
											...
											<c:otherwise>
											<c:forEach items="${projectCostList}" var="v" varStatus="tag">
