...
<head>
	...
	<script type="text/javascript">
		...
		function add(){
			var url = "<%=path %>/financialReport.do?method=addOrEditIncomePage";
		...
		function edit(){
			...
			var url = "<%=path %>/financialReport.do?method=addOrEditIncomePage&operateIncome.id="+id;
			var options = "dialogWidth:800px;dialogHeight:600px;status:no;scroll:no;";
			var win = window.showModalDialog(url, this.window, options);
			if(win=="flag"){
				query();
		...
		function query(){
			document.form1.action="<%=path %>/financialReport.do?method=getIncomeList";
		...
		function del(){
			...
			if(confirm("确定要删除这"+k+"条记录？")){
				document.form1.action = "<%=path %>/financialReport.do?method=delIncomeById&ids="+str;
				document.form1.submit();
			}
		}
	</script>
    		...
    		<td>
    			<table width="100%" border="0" cellspacing="0" cellpadding="0">
      				<tr>
						...
						<td width="165" nowrap="nowrap" class="form_line">财务报表-经营收入列表</td>
		  					...
		  					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					          	...
					          	<tr>
					           		<td class="txt" nowrap="nowrap" width="30%" align="center">
					           			年度：<input type="text" name="operateIncome.year" value="${from.operateIncome.year}" style=" width: 138px" onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy'})" readonly onclick="WdatePicker();" class="Wdate">
					           		</td>
					           		<td class="txt" nowrap="nowrap" width="30%" align="center">
					           			月份：<input type="text" name="operateIncome.month" value="${form.operateIncome.month}" style=" width: 138px" onclick="WdatePicker({skin:'whyGreen',dateFmt:'MM'})" readonly onclick="WdatePicker();" class="Wdate">
					           		</td>
					        		...
					        		<div id = "div1" class = "RptDiv"  >
							   			<table border="0" cellpadding="0" cellspacing="0" class = "RptTable">
					              			<tr>
												...
												<th width="10%" nowrap="nowrap" class="RptTableHeadCellLock" align="center">类型</th>
											</tr>
											<c:choose>
											<c:when test="${empty operateIncomeList}">
											<tr align="center">
											...
											<c:otherwise>
											<c:forEach items="${operateIncomeList}" var="v" varStatus="tag">
											<tr onmouseover="this.className = 'hover';" onmouseout="this.className = '';">
												...
												<td width="" nowrap="nowrap" class="RptTableBodyCell" align="center">&nbsp;${v.moneyType}</td>
											</tr>
											</c:forEach>
											</c:otherwise>
											</c:choose>
