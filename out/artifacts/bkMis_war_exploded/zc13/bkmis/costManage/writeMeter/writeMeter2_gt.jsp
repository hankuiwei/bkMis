	...
	<script type="text/javascript">
		...
		function editTd(obj,str){
			try{
				...
				if(now_value!=old_value){
					...
					tdObj.parentElement.style.backgroundColor = '#20ffc0';
					old_value = now_value;
					var temp = true;
		...
		function cancelEdit(ids){
			try{
				...
				if(now_value!=old_value){
					tdObj.parentElement.firstChild.innerHTML = "*"+tdObj.parentElement.rowIndex;
					//tdObj.parentElement.className = 'hover';
					tdObj.parentElement.style.backgroundColor = '#20ffc0';
					old_value = now_value;
					var temp = true;
					//将修改过的行号保存到modifyRows中，重复的将不再重复保存
		...
		function explortExcel(){
			window.location.href = "<%=path%>/meterInput.do?method=explortPublicExcel";
		}
		...
		//查询
		function query(){
			document.roomform.action = "<%=path%>/meterInput.do?method=getPublicReadMeter";
									...
									<table width="100%" border="0" cellspacing="0" cellpadding="0">
							         	...
							         	<tr>
							            	<td height="10" colspan="3"> 
							            		&nbsp;&nbsp;期间：${years }年${months }月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;公摊表具：${meterName} 
							            	</td>
							            	<td colspan="2">
