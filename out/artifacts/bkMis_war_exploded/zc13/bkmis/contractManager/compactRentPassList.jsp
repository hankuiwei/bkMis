	...
	<script type="text/javascript">
		...
		function return1(str){
			...
			if(window.confirm("确定"+flag+"吗?")){
				window.location.href("<%=path%>/compact.do?method=noapplyChangeOrRelet&type=relet&flag="+str+"&compactId="+compactId+"&id="+id);
			}
		}
		function check(str){
			k = 0;
		...
		function edit(id1,id){
			window.open("<%=path%>/customer.do?method=gotoEditCompact&flag=lookRelet&id="+id+"&id1="+id1);
		}
		function selectAny(){
    		...
    		<td>
    			<table width="100%" border="0" cellspacing="0" cellpadding="0">
      				<tr>
						...
						<td width="165" nowrap="nowrap" class="form_line">待审批的续租合同列表</td>
					          	...
					          	<tr>
					            	...
					            	<td height="10" nowrap="nowrap">合同类型：<input type="hidden" name="typeIn" id="typeIn" value="${compactManagerForm.type }">
										<select name="type" id="type" onchange="selectAny()">
											...
											<option value="<%=Contants.COMPACTRELET %>" selected="selected"><%=Contants.COMPACTRELET %></option>
								...
								<tr height = "95%">
					        		...
					        		<div id = "div1" class="RptDiv">
							   			<table border="0" cellpadding="0" cellspacing="0" class="RptTable">
											...
											<c:choose>
											<c:when test="${empty compactManagerForm.compactRelets}">
											<tr align="center">
											...
											<c:otherwise>
											<c:forEach items="${compactManagerForm.compactRelets}" var="v" varStatus="tag">
											<tr onmouseover="this.className = 'hover';" onmouseout="this.className = '';" ondblclick="edit(${v.compactByNewId.id },${v.id })">
											 	<td width="" nowrap="nowrap" class="RptTableBodyCellLock" align="center">
											 		...
											 		<input type="hidden" id="compactId" value="${v.compactByNewId.id }">
												</td>
								                <td nowrap="nowrap" class="RptTableBodyCellLock" align="center">${tag.count }</td>
								                <td nowrap="nowrap" class="RptTableBodyCell" align="center">${v.compactByNewId.name }&nbsp;</td>
