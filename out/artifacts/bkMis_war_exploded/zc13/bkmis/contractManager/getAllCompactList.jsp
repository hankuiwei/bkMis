...
<head>
	...
	<script type="text/javascript">
		...
		function selectAny(){
			document.form1.action = "compact.do?method=getAllCompactList";
		...
		function calculateAll(){
			...
			if(!window.confirm("您确定要汇总全部合同吗？这样导致系统运行缓慢建议在系统闲暇时进行该操作!")){
				return;
			}
			$.post("<%=path%>/calculate.do",{method:"calculateCompact"},function(data){
				if(data=="1"){
					alert("汇总成功！");
				}else{
					alert("汇总失败！");
		...
		function calculateOne(){
			...
			var type = document.getElementById(id+"type").innerText;
			if(type == "变更合同"){
				alert("不能对变更合同进行汇总!");
				return ;
			}
			$.post("<%=path%>/calculate.do",{method:"calculateCompact",compactId:id},function(data){
				if(data=="1"){
					alert("汇总成功！");
				}else{
					alert("汇总失败！");
							...
							<table  width="100%" height = "100%" border="0" cellspacing="0" cellpadding="0" style="table-layout:fixed" >
								<tr height = "95%">
					        		...
					        		<div id = "div1" class = "RptDiv"  >
							   			<table border="0" cellpadding="0" cellspacing="0" class = "RptTable">
											...
											<c:otherwise>
											<c:forEach items="${compactList}" var="v" varStatus="tag">
											<tr onmouseover="this.className = 'hover';" onmouseout="this.className = '';" ondblclick="edit(${v.id })">
												...
												<td width="6%" nowrap="nowrap" class="RptTableBodyCell" align="center"><span id="${v.id }type">${v.type }</span>&nbsp;</td>
								...
								<tr>
					               	<td nowrap="nowrap" align="right">
					               		<input type="button" onclick="calculateAll()" class="button" value="汇总合同(全部)">
					               		<input type="button" onclick="calculateOne()" class="button" value="汇总合同(单份)">
					               	</td>
					            </tr>
							</table>
    					</td>
