...
<head>
	...
	<script type="text/javascript">
		function query(){
			document.actionForm.action="<%=path%>/meterInput.do?method=showPublicWriteMeterInfo";
		...
		function writeMeter(lpId,meterTypeCode){
			...
			var url = "<%=path%>/zc13/bkmis/costManage/writeMeter/writeMeter1_gt.jsp";
		...
		//双击记录调用的方法
		function query2(years,meterId){
			var url="<%=path%>/meterInput.do?method=getPublicReadMeterByYearAndId&years="+years+"&meterId="+meterId;
		...
		function query3(){
			...
			var url="<%=path%>/meterInput.do?method=getPublicReadMeterByYearAndId"+values;
			window.open (url, '抄录信息', 'height=650, width=1000, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no') //这句要写成一行
		}
					        		...
					        		<td>
							   			...
							   			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="RptTable">
					              			<tr>
								                ...
								                <th width="24%" nowrap="nowrap" class="RptTableHeadCellLock" align="center">位置</th>
								                <th width="24%" nowrap="nowrap" class="RptTableHeadCellLock" align="center">年度</th>
												<th width="15%" nowrap="nowrap" class="RptTableHeadCellLock" align="center">年度用量</th>
											</tr>
											<c:choose>
												...
												<c:otherwise>
													<c:forEach items="${list}" var="c" varStatus="vs">
														<tr onmouseover="this.className = 'hover';" onmouseout="this.className = '';" ondblclick="query2('${c.years }','${c.meterId }');">
															<td class="RptTableBodyCellLock"  align="center"><input type="checkbox" name="checkbox" onclick="selectOne(this);" value="&years=${c.years }&meterId=${c.meterId }"></td>
															<td class="RptTableBodyCellLock"  align="center">${vs.index+1 }</td>
															<td class="RptTableBodyCell">${c.code }</td>
															<td class="RptTableBodyCell">${c.lpName }<c:if test="${c.buildName!=''}">—>${c.buildName}</c:if></td>
															<td class="RptTableBodyCell">${c.years }</td>
															<td class="RptTableBodyCell">${c.totals }</td>
														</tr>
													</c:forEach>
												</c:otherwise>
											</c:choose>
